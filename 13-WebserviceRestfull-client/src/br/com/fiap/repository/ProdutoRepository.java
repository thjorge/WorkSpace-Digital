...
/**
 * Tipo o DAO mas pode manipular 
 * informações de outro lugares (Webservices, Banco)
 *  * @author Thjorge
 *	@version 1
 */
public class ProdutoRepository {
	...
	public void atulizar(ProdutoTO produto) throws WebServiceException{
	...
	public List<ProdutoTO>listar() throws WebServiceException{
		...
		if(response.getStatus() !=200){
			throw new WebServiceException("HPPT Status: " + response.getStatus());
		}
	...
	public void cadastrar(ProdutoTO produto) throws WebServiceException {
		...
		WebResource resource = cliente.resource(URL);
		/*
		 * Configura que tipo de dado será enviado e
		 *  faz a requisição enviando o produto
		 */
		ClientResponse response = resource.type(MediaType.APPLICATION_JSON)
